...
set EXTRACTION_PASSWORD=tyM24R83Qc

set MIN_SIZE_REQUIRED=22GB
...
if not exist "C:\Program Files (x86)\VMware\VMware Workstation\vmware.exe" (
	if not exist "C:\Program Files (x86)\VMware\VMware Workstation\vmplayer.exe" (
		if not exist "C:\Program Files (x86)\VMware\VMware Player\vmplayer.exe" (
			...
			start /wait VMware-workstation-full-17.6.3-24583834.exe /s /v/qn EULAS_AGREED=1 INSTALLDIR="C:\Program Files (x86)\VMware\VMware Workstation\"
...
rem FOR THIS VERSION OF THE SCRIPT cd up one directory before install
set USB_DRIVE=%cd%\..\
...
FOR /F "usebackq delims=" %%i in (`dir /s /b VMS_TO_INSTALL\*mint*.zip`) DO (
		set fullpath=%%~i
		if "!fullpath:~-3!"=="zip" (			
...
echo "Installation Completed.  You may now safely remove the thumb drive"
pause