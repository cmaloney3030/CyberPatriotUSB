...
if exist "%LOCAL_DIR%"\Files\.updated (
		...
		if "!FILENAME!" == "!last_updated!" (
			...
			if !last_updated_counter! GEQ 5 (
				...
				if "%~1"=="--post-update" (
					...
					echo [92m   No need to update....number !last_updated_counter! of 5 [0m
					goto :main_logic
				)
			)
		)
...
for /r "%LOCAL_DIR%" %%F in (*) do (
    ...
    if /i "!FULL_PATH!" neq "%~f0" (
        ...
        :: If it wasn't found in the manifest, purge it, ignore VMS_TO_INSTALL, .updated files, and System Volume Information
        if "!IS_TRACKED!"=="0" (
			if "!RELATIVE_PATH:VMS_TO_INSTALL=!"=="!RELATIVE_PATH!" (
				if "!RELATIVE_PATH:.updated=!"=="!RELATIVE_PATH!" (
